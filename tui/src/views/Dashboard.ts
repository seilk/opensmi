...
export function renderSlurmClusterTab(slurmIdx: number) {
  ...
  const footer = Box(
    ...
    Text({ content: t`${fg(C.textDim)("[[]/[]]")} Switch  ${fg(C.textDim)("[Tab]")} Next  ${fg(C.textDim)("[↑↓/jk]")} Scroll  ${fg(C.textDim)("[Enter]")} Popup  ${fg(C.textDim)("[Double-click]")} Open  ${fg(C.textDim)("[r]")} Refresh` }),
  );
