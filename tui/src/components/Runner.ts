 ...
 * Contains: renderRunnerPane, renderGpuAssignmentPanel, launch/execution functions,
...
// ── GPU Assignment Panel ───────────────────────────────────────────

export function renderGpuAssignmentPanel() {
  if (!S.snapshot) {
    return Box(
  ...
  let modeText = S.launchGpuMode === "auto"
    ? "(Auto-ranked)              [click to exclude]  [g] manual"
    : "(Manual selection)         [click to toggle]  [g] auto";

  if (S.launchSourceBundle) {
    modeText = `(From bundle: ${S.launchSourceBundle})         [g] auto`;
  }
  ...
  for (const node of S.snapshot.nodes) {
    for (const gpu of node.gpus) {
      ...
      const alloc = S.allocations.find(a => a.node_alias === node.node_alias && a.gpu_index === gpu.index);
      gpuInfoMap.set(key, {
        gpu,
        node,
        allocated: !!alloc,
  ...
  if (S.launchSelectedGpus.length > 0) {
    for (let i = 0; i < S.launchSelectedGpus.length; i++) {
      const selectedGpu = S.launchSelectedGpus[i]!;
      ...
      const idleStartTime = S.gpuIdleStart[gpuIdleKey] || Date.now();
      ...
      const isSelected = true; // All GPUs in S.launchSelectedGpus are selected
      ...
      // Build line content
      const linePrefix = S.launchDistMode === "one-to-one"
        ? `Command ${i + 1} → `
        : `  `;
        ...
        Box({
          ...
          onMouseUp: (e: any) => {
            ...
            if (S.launchGpuMode === "auto") {
              ...
              if (idx === -1) {
                S.launchExcludedGpus.push({ node: selectedGpu.node, gpu: selectedGpu.gpu });
                ...
                setTimeout(() => {
                  refreshLaunchGpuSelection().then(() => {
                    if (S.requestRender) S.requestRender();
                  });
                }, 10);
              }
            } else {
  ...
  const numSelected = S.launchSelectedGpus.length;
  const numWithConflicts = S.launchSelectedGpus.filter((gpu) => {
  ...
  const helpText = S.launchGpuMode === "auto"
