...
| 탭 | 단축키 | 설명 |
|----|--------|------|
| 대시보드 | `1` | 실시간 GPU 그리드 — 노드별 사용 현황 |
| 노드 상세 | `2` | GPU별 메모리, 사용률, 프로세스 목록 |
| 작업 | `3` | 대기·실행·완료 작업 추적 |
| 할당 | `4` | GPU→사용자 할당 조회 및 관리 |
| 설정 | `5` | 노드별 환경 설정 (conda, venv, 작업 디렉토리) |

**Command Runner**는 탭이 아닌 하단 고정 pane입니다. `Ctrl+X ↓`로 포커스합니다.

### 실행기 (Command Runner)

화면 하단에 항상 표시되는 pane입니다. `Ctrl+X ↓`로 포커스합니다.

| 단축키 | 동작 |
|--------|------|
| `Ctrl+X ↓` | 실행기 pane 포커스 |
| `Ctrl+X F` | pane 접기 / 펼치기 |
| `Enter` | 명령어 입력 시작 |
| `Ctrl+X Enter` | 실행 |
| `Esc` | 포커스 해제 |

GPU 작업을 자동 GPU 선택 및 오케스트레이션과 함께 실행합니다.
